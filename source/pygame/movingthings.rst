...
At this point you should have basic knowledge how to make pygame initialization,
drawing simple graphics, showing (blitting) it on a screen and running
your game within a game loop. If not, please do the :ref:`pygameintroduction`
chapter.
...
#. Number between ``@@``-markers is the line number you need to locate from the
   source code.
...
#. Read lines in the order of appearance and make a note of the character in
   the first column. Character can be one of the following: minus, plus or a space.
...
#. Notation may have multiple ``@@ <lineno> @@`` markers. When you encounter
   a new marker locate the line number from your file. Note that the line number
   is based on the previous removals or additions.

Don't worry if notation feels strange initially. Full code is after every
section to help verify that you have done things right and get working code
if you didn't manage to type code correctly.


Moving object theory
--------------------

Moving an object is a straightforward. You keep track of an object position
on screen and on every game loop you draw object at given position. You already
did this part when making your square to jump around screen in introduction 
chapter.
...
common ways to move object. By moving object using some keys. Very common is
to use so called WASD movement or cursor keys. After that you will learn to
move object based on mouse click. Finally you will learn to follow the mouse.
...
Make sure that you have the same file in your editor and saved file file.

Moving object with keys
-----------------------

In this section you will be using pygame to move and object, the player, by
...
Because movement is time related, meaning that object is moved certain amount
of units, a pixels, per time unit. In this tutorial you will be using time unit
of one frame at the speed of 30 frames per second. It can be translated to
object movement number of units (pixels) per ~33.33 milliseconds.

To limit number of frames per second, also known as framecap or frame limit,
...
This change doesn't seemengly affect program anyways but it changes internal
...
make your rectangle to go up, pressing S will make it go down, pressing A makes
it to go left and pressing D makes it go to right. Since you already do have
``rect`` which you can manipulate - just like you manipulated it to jump in
:ref:`pygameintroduction` chapter.
...
moving with keys. First, you need to track direction where movement happens,
a speed which movement happens and finally yo need to track where your movement
should stop. There are many ways to achieve that. This time you will be using
vectors since they simplify math required. Movement could be done with basic
trigonometry as well.

First you need to declare variables for speed, direction and position. Position
is good to keep in vector since it's floating point value. Rect handles only
integers. Quantizing to int would cause problems in moving, including that 
at low speeds object might not move at all. Speed isn't absolutely necessary
it could be hard coded but it makes code more readable.
...
to. ``direction_vec`` is a bit more complex. First position vector is substracted
from target vector. This gives a vector that points to direction of target
position. But the problem is that length of the vector is actually distance
from current position to target position. To fix that vector is normalized,
made as length of one. Now you have nice vector that is length of one and
points to direction where object needs to move to reach target.
...
Try also changing speed value to bigger or smaller to see how it affects
movement.
